...
class PreviewQLabel(QLabel):
    ...
    def mousePressEvent(self, event):
        if self.scale_factor is not None:
            self.mask_points.append((int(round(event.x()/self.scale_factor)), int(round(event.y()/self.scale_factor))))

            self.preview_window.draw_mask_points(self.mask_points)

    def mouseMoveEvent(self, event):
        if self.scale_factor is not None:
            self.preview_window.draw_mask_points(self.mask_points + [(int(round(event.x()/self.scale_factor)), int(round(event.y()/self.scale_factor)))])
        ...
        if self.drawing_mask:
            ...
            if n_points >= 1:
                for i in range(n_points):
                    if i < n_points - 1:
                        cv2.line(image, mask_points[i], mask_points[i+1], (255, 0, 0), 1)
                    cv2.circle(image, mask_points[i], 2, (255, 0, 0), -1)
